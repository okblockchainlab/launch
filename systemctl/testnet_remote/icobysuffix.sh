...
issue() {
    ${OKCHAIN_CLI} tx token issue --from captain --symbol ${1} --chain-id okchain \
        -n 80000000000 --mintable=true -y --node ${TESTNET_RPC_INTERFACE} -w ${1} -b block
}
...
issue_by_suffix() {
    ...
    do
         echo "okchaincli tx token issue --desc \"发币 ${token}\" --mintable --symbol $token -w $token -n 80000000000 --node ${TESTNET_RPC_INTERFACE} --from captain -y -b block"
        okchaincli tx token issue --mintable --symbol $token -n 80000000000 \
            --node ${TESTNET_RPC_INTERFACE} \
            --desc "发币 ${token}" \
            -w $token \
            -b block \
